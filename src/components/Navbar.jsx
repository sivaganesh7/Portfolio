...
const Navbar = () => {
  ...
  useEffect(() => {
    let ticking = false;
    const sectionsReversed = navLinks.map(link => link.href.slice(1)).reverse();
        ...
        >
          {navLinks.map((link) => (
            <button
              ...
              className={`relative font-body text-sm tracking-wide transition-colors duration-300 ${activeSection === link.href.slice(1)
                  ? 'text-accent-primary'
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="fixed top-0 right-0 w-72 h-screen bg-white/95 backdrop-blur-xl border-l border-accent-primary/10 flex flex-col justify-center items-center gap-8 z-40 shadow-2xl"
            >
              {navLinks.map((link, i) => (
                <motion.button
                  key={link.href}
